...
while True:
    ...
    if comanda == 'EXIT':
        ...
        if not este_conectat:
            ...
            continue
        if len(parti) < 2 or not parti[1] or parti[1].isspace():
            print("EROARE: Mesajul nu poate fi gol. Sintaxa: PUBLISH <mesaj>")
            continue
        raspuns = trimite_comanda(intrare)
        print(raspuns)
        ...
        if len(parti) < 2 or not parti[1]:
            print("EROARE: Trebuie sa furnizezi un ID. Sintaxa: DELETE <id>")
            continue
        try:
            int(parti[1])
        except ValueError:
