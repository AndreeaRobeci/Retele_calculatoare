    ...
    try:
        ...
        if comanda == 'CONNECT':
            ...
            if adresa_client not in clienti_conectati:
                raspuns = "EROARE: Nu esti conectat la server."
            elif not argumente or argumente.isspace():
                raspuns = "EROARE: Mesajul nu poate fi gol."
            else:
                contor_id += 1
                mesaje[contor_id] = {'text': argumente, 'author': adresa_client}
                raspuns = f"OK: Mesaj publicat cu ID={contor_id}"
                print(f"[SERVER] Mesaj publicat cu ID={contor_id} de {adresa_client}")
            ...
            if adresa_client not in clienti_conectati:
                ...
                try:
                    ...
                    if msg_id not in mesaje:
                        raspuns = "EROARE: ID-ul mesajului nu exista."
                    elif mesaje[msg_id]['author'] != adresa_client:
                        raspuns = "EROARE: Nu poti sterge mesajul altuia."
                    else:
                        del mesaje[msg_id]
                        raspuns = f"OK: Mesajul cu ID={msg_id} a fost sters."
                        print(f"[SERVER] Mesaj cu ID={msg_id} sters de {adresa_client}")
                except ValueError:
            ...
            if adresa_client not in clienti_conectati:
                ...
                raspuns = "OK: Mesaje:\n"
                for msg_id, msg_data in sorted(mesaje.items()):
                    raspuns += f"  ID={msg_id}: {msg_data['text']}\n"
                raspuns = raspuns.rstrip()
